...
kore_mysql_init(void)
{
	...
	LIST_INIT(&mysql_db_conn_strings);
...
kore_mysql_query(struct kore_mysql *mysql, const char *query)
{
	...
	if (mysql->flags & KORE_MYSQL_SYNC) {
		...
		if ((PQresultStatus(mysql->result) != PGRES_TUPLES_OK) &&
		    (PQresultStatus(mysql->result) != PGRES_COMMAND_OK)) {
...
int
kore_mysql_v_query_params(struct kore_mysql *mysql,
    const char *query, int result, u_int8_t count, va_list args)
{
	...
	if (mysql->flags & KORE_MYSQL_SYNC) {
		mysql->result = PQexecParams(mysql->conn->db, query, count,
		...
		if (!PQsendQueryParams(mysql->conn->db, query, count, NULL,
    ...
    const char *query, int result, u_int8_t count, ...)
{
	...
	ret = kore_mysql_v_query_params(mysql, query, result, count, args);
...
{
	struct mysql_wait	*pgw, *next;

	for (pgw = TAILQ_FIRST(&mysql_wait_queue); pgw != NULL; pgw = next) {
		next = TAILQ_NEXT(pgw, list);
		if (pgw->req != req)
			continue;

		TAILQ_REMOVE(&mysql_wait_queue, pgw, list);
		kore_pool_put(&mysql_wait_pool, pgw);
		return;
	}
}
...
{
	struct mysql_wait	*pgw;

	http_request_sleep(req);

	pgw = kore_pool_get(&mysql_wait_pool);
	pgw->req = req;
	pgw->req->flags |= HTTP_REQUEST_MYSQL_QUEUE;

	TAILQ_INSERT_TAIL(&mysql_wait_queue, pgw, list);
}
